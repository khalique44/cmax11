    ...
    $.fn.formError =
        function( message, options)
        {
            ...
            function remove( options)
            {
                ...
                if( options.successImage.enabled)
                {
                    if( this.hasClass('invalid')) //Used to be invalid
                    {
                        ...
                        {
                            img
                                .css( "right", ((this.position().right + this.outerWidth()) + 3) + "px")
                                .css( "top", (this.position().top - 3) + "px");
                        }
                        else
                        {
                            img
                                .css( "right", ((this.position().right + this.outerWidth()) + 5) + "px")
                        }
            ...
            function show( options)
            {
                ...
                var errorDiv =
                    $("<div class='validationErrorContainer' style='position:absolute; right:101%; top:-2px; z-index:99999;'>" +
                        "<canvas width='14' height='14' style='position:absolute; right:-3px; top:7px;' />" +
                        "<div class='validationError' style='border:2px solid #fdffe0; border-radius:5px; padding:4px; background-color:#060404; color:#fdfcff; position:relative; right:10px; white-space:nowrap;'>" +
                        options.message +
                        "</div>" +
                        "</div>");
                ...
                if( typeof( canvas.getContext) == "function")
                {
                    ...
                    context.fillStyle = '#060404';
                    context.strokeStyle = '#060404';
                    context.lineWidth = 1;
